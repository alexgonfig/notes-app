...
async def register(user: UserCreate, db_session: AsyncSession = Depends(get_db_session)) -> UserResponse:
    return await register_user(user, db_session)


# User login
@router.post("/login")
async def login(user: UserLogin, db_session: AsyncSession = Depends(get_db_session)) -> UserLoginResponse:
    return await authenticate_user(user, db_session)


# validate user token and return user data
@router.get("/validateToken")
async def validate(db_session: AsyncSession = Depends(get_db_session), token_payload: dict = Depends(validate_auth)) -> UserResponse:
